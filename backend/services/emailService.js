...
/**
 * Send an email using Brevo's Transactional Email API (HTTPS, port 443).
 * This bypasses SMTP port restrictions on DigitalOcean.
 */
...
export const sendAppointmentRequestEmail = async (facultyEmail, facultyName, appointmentData, emailActionToken) => {
  try {
    ...
    const result = await sendViaBrevoDB(facultyEmail, 'Yeni Randevu Talebi', html);
    if (result.success) console.log(`✅ Appointment request email sent to ${facultyEmail}`);
    return result;
  } catch (error) {
...
export const sendAppointmentApprovalEmail = async (studentEmail, studentName, appointmentData, googleMeetLink = null) => {
  try {
    ...
    const result = await sendViaBrevoDB(studentEmail, 'Randevu Talebiniz Onaylandı', html);
    if (result.success) console.log(`✅ Appointment approval email sent to ${studentEmail}`);
    return result;
  } catch (error) {
...
export const sendAppointmentRejectionEmail = async (studentEmail, studentName, appointmentData, rejectionReason) => {
  try {
    ...
    const result = await sendViaBrevoDB(studentEmail, 'Randevu Talebiniz Reddedildi', html);
    if (result.success) console.log(`✅ Appointment rejection email sent to ${studentEmail}`);
    return result;
  } catch (error) {
...
export const sendAppointmentCancellationEmail = async (studentEmail, studentName, appointmentData, cancellationReason) => {
  try {
    ...
    const result = await sendViaBrevoDB(studentEmail, 'Randevunuz İptal Edildi', html);
    if (result.success) console.log(`✅ Appointment cancellation email sent to ${studentEmail}`);
    return result;
  } catch (error) {
...
export const sendPasswordResetEmail = async (toEmail, toName, resetLink) => {
  try {
    ...
    const result = await sendViaBrevoDB(toEmail, `${appName} | Şifre Sıfırlama`, html);
    if (result.success) console.log(`✅ Password reset email sent to ${toEmail}`);
    return result;
  } catch (error) {
...
export const sendTemporaryPasswordEmail = async (toEmail, toName, tempPassword, loginUrl) => {
  try {
    ...
    const result = await sendViaBrevoDB(toEmail, `${appName} | Geçici Şifreniz`, html);
    if (result.success) console.log(`✅ Temporary password email sent to ${toEmail}`);
    return result;
  } catch (error) {
...
export const sendEmailVerificationCode = async (toEmail, toName, verificationCode) => {
  try {
    ...
    const result = await sendViaBrevoDB(toEmail, `${appName} | E-posta Doğrulama Kodu`, html);
    if (result.success) console.log(`✅ Email verification code sent to ${toEmail}`);
    return result;
  } catch (error) {
